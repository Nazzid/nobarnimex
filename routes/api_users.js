...
router.get('/changeVideo', function(req, res, next) {
	...
	if (res.user.isHost == false) {
        res.json({
            error: true,
            message: 'Hanya host yang dapat mengganti Link Video!'
        });
    ...
    shared.rooms[res.user.roomId].videoURL = req.query.videoFileName;
    ...
    // Broadcast
    shared.io.to(res.user.roomId).emit('videoChanged', {
        message: req.query.videoFileName
    });
    ...
    res.json({
        ...
        users: _.chain(shared.users)
				...
				.map(function (user) {
					...
					userIsHost = false;
					if(shared.rooms[res.user.roomId].hostId == user.userId){
						userIsHost = true;
					}
